...
| `hubble`(动态) | — | **宇宙年龄 t(z)**（由 H(z) 积分，几何论无暗物质 → 17.3 Gyr 可检验差异） |
| `satellite`(动态) | 8.12 | **地月距离 a(t) 演化**（积分退行 3.8 cm/年，潮汐锁定时代 9.7e9 年） |
| `solar`(动态) | 8.8 | **水星近日点进动 ω(t)**（42.98 角秒/世纪，观测 43.0，偏差 <0.05%） |
| `cmb`(动态) | 8.5 | **早期宇宙 T(t)/T(z)** + 复合时代 + BBN 参数（η=6.1e-10、N_ν=3） |
