...
ax[1].set_ylabel("偏差 (%)")
ax[1].set_title("几何论 vs galpy 旋转速度偏差")
plt.tight_layout()
...
vT_phys = gvcirc(MWPotential2014, R_SUN_KPC / G_RO) * G_VO * 0.99  # km/s（物理）
o_galpy = Orbit([R_SUN_KPC, 0.0, 0.0, vT_phys, 0.0, 0.0])  # 物理单位（kpc, km/s）
ts_orb = np.linspace(0, 3.0, 400)  # 0-3 Gyr
...
# galpy 轨道投影
xg, yg = o_galpy.x(0) * 0, o_galpy.x(0) * 0  # 占位
xg = np.array(o_galpy.x(0)) * 0
# galpy orbit 的 x(t) 需用 ts
xgal = np.array(o_galpy.x(ts_orb))  # 物理 kpc
...
# galpy 周期
import numpy as np
r_gal = np.array([np.hypot(o_galpy.x(t), o_galpy.y(t)) for t in ts_orb])
# 几何论周期（已知函数）
